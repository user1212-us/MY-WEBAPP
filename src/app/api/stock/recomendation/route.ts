...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const apiResponse = await fetch(
      `https://financialmodelingprep.com/api/v3/analyst-stock-recommendations/${symbol}?apikey=${process.env.MY_API_KEY}`,
      { cache: "no-store" }
    );

    // If the response is not okay, throw an error
    if (!apiResponse.ok) {
      throw new Error("Failed to fetch recomendation data");
    }
